void draw(){
	...
	int rebin=1; 
	TFile *fDYjet = TFile::Open("DYjet.root") ;

	
	TString histname = "h1_Mee"; XMAX=200; XMIN=0; rebin=100; YMAX=10000; TString title_name = "Mass_{ee}";


	TH1F *hDYjet = (TH1F*)fDYjet->Get(histname); 
	
	hDYjet->SetLineWidth(3); hDYjet->SetLineColor(38); 
	hDYjet->Rebin(rebin);
